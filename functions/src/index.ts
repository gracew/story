...
export const sendVideoLink = functions.pubsub.schedule('55 * * * *').onRun(async (context) => {
    ...
    await admin.firestore().runTransaction(async txn => {
        ...
        videoMatches.forEach(doc => {
            const userA = usersById[doc.get("user_a_id")];
            const userB = usersById[doc.get("user_b_id")];
            const bodyA = `Hi ${userA.firstName}! You can join the video call in a few minutes at https://storydating.com/v/${doc.get("videoId")}/a. In case you need it, the passcode is ${doc.get("videoPasscode")}. Happy chatting!`;
            const bodyB = `Hi ${userB.firstName}! You can join the video call in a few minutes at https://storydating.com/v/${doc.get("videoId")}/b. In case you need it, the passcode is ${doc.get("videoPasscode")}. Happy chatting!`;
...
export const revealRequestVideo = functions.pubsub.schedule('0 * * * *').onRun(async (context) => {
    ...
    await admin.firestore().runTransaction(async txn => {
        ...
        await Promise.all(videoMatches.map(async doc => {
            await callStudio("reveal_request", doc.data() as IMatch, new Firestore(), true);
            txn.update(doc.ref, "revealRequested", true);
        }));
    });
});
