...
it("videoNextStep - match", async () => {
    ...
    const m = match(userA.id, userB.id, "2021-04-17T20:00:00-07:00", { videoAvailability });
    ...
    expect(mockSendSms.mock.calls[0][0].body).toContain("you'll be speaking again");
    expect(mockSendSms.mock.calls[1][0].to).toEqual(userB.phone);
    expect(mockSendSms.mock.calls[1][0].body).toContain("you'll be speaking again");
});
